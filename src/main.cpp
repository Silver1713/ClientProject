...
struct SocketFileDownloadSender : public SocketFileDownload, sender_state {
	...
	void send_seq(ULONG seq_to_send) {
		...
		if (((seq_to_send + 1) * (BUFFER_SIZE - 13)) > data_size) {
			remaining_data_size = static_cast<ULONG>(data_size - (seq_to_send * (BUFFER_SIZE - 13)));
			...
			ULONG data_size_net = htonl(static_cast<ULONG>(data_size));
	...
	void send() {
		ULONG max_seq_size = static_cast<ULONG>((data_size / (BUFFER_SIZE - 13)) + 1);
		seq_sent = 0;
...
struct SocketFileDownloadReceiver : public SocketFileDownload, receiver_state {
	...
	bool receive_packet() {
		...
		int bytesReceived = recvfrom(udp_socket, buffer, BUFFER_SIZE, 0, (sockaddr*)(&destination), &size);
		...
		// Verify session ID
		ULONG received_session_id = ntohl(*(ULONG*)(buffer));
...
void asyncInputHandler(SOCKET clientSocket, const std::string& downloadPath) {
	...
	while (!stop_thread) {
		...
		if (buffer[0] == CMDID::RSP_DOWNLOAD) {
			...
			std::string filename = "download_" + std::to_string(session_id) + ".bin";
...
int main(int argc, char** argv)
{
	...
	udpAddr.sin_port = htons(std::stoi(clientUdpPortStr));
	...
	while (std::getline(std::cin, command)) {
		...
		if ((command_l = command.substr(0, 2)) == "/d") {
			...
			std::string file_name = command.substr(space_pos + 1);
